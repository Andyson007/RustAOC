...
fn main() {
    let input = include_str!("../../input")
        ...
        .collect::<Vec<Oper>>();
    let hashed = "fbgdceah";
    let mut arrangement = Arrangement::<8>::from(hashed);

    let ans = loop {
        let prev = format!("{arrangement}");
        for line in input.iter() {
            arrangement.modify(&line);
        }
        if format!("{arrangement}") == hashed {
            break prev;
        }
    };
    println!("{ans}");
...
impl<const N: usize> Arrangement<N> {
    ...
    fn from(x: &str) -> Self {
        Self {
            arrangement: x.chars().collect::<Vec<char>>().try_into().unwrap(),
            rotate: 0,
        }
    }
}
