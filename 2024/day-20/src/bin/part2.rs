...
fn solve<const M: usize>(raw: &str) -> usize {
    ...
    dists
        ...
        .map(|(curr, base_score)| {
            let mut count = 0;
            for (other_position, other_score) in &dists {
                let dist = curr.dist(*other_position);
                if dist <= 20 && *other_score >= *base_score + dist + M {
                    count += 1
                }
            }
            count
...
mod test {
    ...
    #[test]
    fn example() {
        assert_eq!(solve::<56>(include_str!("../../example")), 753753573);
    }
}
