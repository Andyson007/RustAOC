...
fn main() {
    ...
    let mut input = None;
    let mut values = HashMap::from([((0, 0), 1)]);
    ...
    let min_x = values.iter().map(|x| x.0 .0).min().unwrap();
    let max_x = values.iter().map(|x| x.0 .0).max().unwrap();
    let min_y = values.iter().map(|x| x.0 .1).min().unwrap();
    let max_y = values.iter().map(|x| x.0 .1).max().unwrap();
    for y in (min_y..=max_y).rev() {
        for x in min_x..=max_x {
            if values.get(&(x, y)) == Some(&1) {
                print!("█");
            } else {
                print!(" ");
            }
