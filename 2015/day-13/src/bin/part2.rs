...
fn main() {
    ...
    for arrangement in generateswaps(names) {
        max = cmp::max(eval_seating(&arrangement, &vals), max);
    }
    println!("{max}");
}

fn eval_seating(names: &Vec<&&str>, weights: &HashMap<(&str, &str), i32>) -> i32 {
    names
        .iter()
        .circular_tuple_windows()
        .map(|(a, b)| weights.get(&(a, b)).unwrap_or(&0) + weights.get(&(b, a)).unwrap_or(&0))
        .sum::<i32>()
}
