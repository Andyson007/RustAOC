...
fn main() {
    let ans = solve::<1000>(include_str!("../../input"));
    println!("{ans}");
}

fn solve<const CONNECTIONS: usize>(input: &str) -> usize {
    ...
    for pair in pairs.into_iter().take(CONNECTIONS) {
    ...
    graphs.sort_unstable_by_key(|x| x.nodes.len());
    graphs
        .iter()
        .rev()
        .take(3)
